...
Agents that previously carried `_fired` (*RefundAgent*, *ResolutionAgent*,
*ResponseComposerAgent*) no longer do so. The blackboard gate condition is
sufficient to ensure each agent fires exactly once per request. reset() on
BaseAgent remains a no-op and is kept for interface consistency.
...
- ComplaintAgent finishes and publishes ComplaintResultMessage. 
- The hub calls refund_agent.handle(). 
 
Inside RefundAgent.handle():
```python
if deps.board.purchase is None or deps.board.complaint is None:
    return  # purchase not yet on board — exit silently
```

`hub.publish(ComplaintResultMessage)` looks up `ComplaintResultMessage`. Finds `[refund_handler]`. Calls
...
`hub.publish(PurchaseResultMessage)` looks up `PurchaseResultMessage`. Finds `[refund_handler]`. Calls
...
`hub.publish(RefundResultMessage)` looks up `RefundResultMessage`. Finds `[resolution_handler]`. Calls
...
`hub.publish(ProfileResultMessage)` looks up `ProfileResultMessage`. Finds `[resolution_handler]`. Calls
...
`hub.publish(ResolutionResultMessage)` looks up `ResolutionResultMessage`. Finds `[composer_handler]`. Calls
...
complaint_agent:  deps.board.complaint = ComplaintOutput(...)
                       await hub.publish(ComplaintResultMessage(...))
                             └── refund_agent.handle()
                                   gate check: purchase ✓  complaint ✓  → PASSES
                                   ← still inside _check(), hasn't set _fired yet

purchase_agent:        await hub.publish(PurchaseResultMessage(...))
                             └── refund_agent.handle()
                                   gate check: purchase ✓  complaint ✓  → PASSES AGAIN
```

Both invocations pass the gate. `_check()` runs twice. `RefundResultMessage` is published
...
| ResponseComposerAgent | receives `ResolutionResultMessage` | Yes (defensive) |
