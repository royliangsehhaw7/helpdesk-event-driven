...
to the hub. Phase 2 synthesis agents subscribe to those finding events, check gate conditions
...
When `hub.publish(CustomerMessageEvent)` is called, the `EventBus` does this:
...
from core.messaage_hub import MessageHub
from core.deps import Deps
...
async def handle_customer_message(
    ...
    #
    # buhubs.publish(message) calls asyncio.gather() on all Phase 1 handlers.
...
**Phase 3 — MessageHub** (`cire/message_hub.py`)
